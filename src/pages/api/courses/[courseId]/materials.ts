...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      let materials: CourseMaterial[] = materialsSnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      } as CourseMaterial));
      ...
      if (!isLecturer) {
        return res.status(403).json({ error: 'Only lecturers can create course materials' });
      }
      ...
      const materialRef = await adminDB.collection('courseMaterials').add(materialData);
      ...
      if (!isLecturer) {
        return res.status(403).json({ error: 'Only lecturers can update course materials' });
      }

      const { materialId, title, description, fileAttachment, isVisible } = req.body;
      ...
      const materialDoc = await adminDB.collection('courseMaterials').doc(materialId).get();
      ...
      if (materialData?.courseId !== courseId) {
        return res.status(403).json({ error: 'Material does not belong to this course' });
      }
      ...
      if (fileAttachment !== undefined) updateData.fileAttachment = fileAttachment;
      if (isVisible !== undefined) updateData.isVisible = isVisible;

      await adminDB.collection('courseMaterials').doc(materialId).update(updateData);
      ...
      if (!isLecturer) {
        return res.status(403).json({ error: 'Only lecturers can delete course materials' });
      }
