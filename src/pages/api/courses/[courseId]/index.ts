  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      try {
        ...
        const lecturerDoc = await adminDB.collection('users').doc(courseData.createdBy).get();
        ...
        lecturerInfo = { uid: courseData.createdBy, name: 'Unknown', email: null };
      }
      ...
      if (isLecturer && courseData.students?.length > 0) {
        ...
        for (const studentId of courseData.students) {
          try {
            ...
            studentsInfo.push({
              uid: studentId,
              name: studentData?.fullName || studentUser.displayName || 'Unknown',
