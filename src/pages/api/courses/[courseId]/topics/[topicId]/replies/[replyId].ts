...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!courseId || typeof courseId !== 'string' || !topicId || typeof topicId !== 'string' || !replyId || typeof replyId !== 'string') {
      return res.status(400).json({ error: 'Invalid course, topic, or reply ID' });
    }
    ...
    // Get reply
    const replyDoc = await adminDB.collection('courseTopics').doc(topicId).collection('replies').doc(replyId).get();
    if (!replyDoc.exists) {
    ...
    if (req.method === 'PUT') {
      ...
      await adminDB.collection('courseTopics').doc(topicId).collection('replies').doc(replyId).update({
        content,
    ...
    if (req.method === 'DELETE') {
      ...
      if (replyData.createdBy !== uid && !isLecturer) {
        return res.status(403).json({ error: 'Only reply creator or lecturer can delete' });
      }
      await adminDB.collection('courseTopics').doc(topicId).collection('replies').doc(replyId).delete();
