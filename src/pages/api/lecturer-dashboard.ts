...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (userData?.role !== 'Lecturer') {
      return res.status(403).json({ error: 'Only lecturers can access dashboard' });
    }

    // 1. Total Groups (created by lecturer)
    const groupsSnapshot = await adminDB.collection('groups').where('createdBy', '==', uid).get();
    const totalGroups = groupsSnapshot.size;

    // 2. Total Students (unique students in lecturer's courses)
    const coursesSnapshot = await adminDB.collection('courses').where('createdBy', '==', uid).get();
    ...
    if (courseIds.length > 0) {
      const topicsSnapshot = await adminDB.collection('courseTopics').where('courseId', 'in', courseIds.slice(0, 10)).get();
    ...
    if (courseIds.length > 0) {
      const assignmentsSnapshot = await adminDB.collection('assignments').where('courseId', 'in', courseIds.slice(0, 10)).get();
      const now = new Date();
