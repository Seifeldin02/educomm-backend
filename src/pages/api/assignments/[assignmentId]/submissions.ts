...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const courseDoc = await adminDB.collection('courses').doc(assignmentData.courseId).get();
    if (!courseDoc.exists) {
    ...
    if (req.method === 'GET') {
      if (isLecturer) {
        ...
        const submissions: Submission[] = await Promise.all(
          submissionsSnapshot.docs.map(async (doc) => {
            ...
            try {
              const studentUser = await adminAuth.getUser(submissionData.studentId);
              const studentDoc = await adminDB.collection('users').doc(submissionData.studentId).get();
              const studentUserData = studentDoc.data();
              
              studentInfo = {
                name: studentUserData?.fullName || studentUser.displayName || 'Unknown',
        ...
        return res.status(200).json({
          success: true,
          submission,
          canSubmit: new Date() <= new Date(assignmentData.dueDate) || assignmentData.allowLateSubmission
        });
      }
      ...
      if (!isEnrolledStudent) {
        return res.status(403).json({ error: 'Only enrolled students can submit assignments' });
      }
      ...
      if (now > dueDate && !assignmentData.allowLateSubmission) {
        return res.status(400).json({ error: 'Assignment deadline has passed and late submissions are not allowed' });
      }

      const { fileAttachment } = req.body;

      if (!fileAttachment) {
        return res.status(400).json({ error: 'File attachment is required for submission' });
      }
      ...
      if (!existingSubmission.empty) {
        return res.status(409).json({ error: 'You have already submitted this assignment. Use PUT to update your submission.' });
      }
      ...
      if (!submissionData || submissionData.assignmentId !== assignmentId) {
        return res.status(403).json({ error: 'Submission does not belong to this assignment' });
      }
      ...
      if (isLecturer) {
        ...
        if (now > dueDate && !assignmentData.allowLateSubmission) {
          return res.status(400).json({ error: 'Cannot update submission after deadline' });
        }

        if (!fileAttachment) {
          return res.status(400).json({ error: 'File attachment is required for submission update' });
        }
      ...
      if (now > dueDate) {
        return res.status(400).json({ error: 'Cannot delete submission after deadline' });
      }
